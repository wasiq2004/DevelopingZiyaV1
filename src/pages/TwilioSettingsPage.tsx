...
const TwilioSettingsPage: React.FC = () => {
  ...
  const loadTwilioAccounts = async () => {
    if (!user?.id) return;
    try {
      const response = await fetch(`${api.getApiBaseUrl()}/twilio/accounts?userId=${user.id}`);
      if (response.ok) {
        const data = await response.json();
        if (data.success) {
  ...
  const handleAddAccount = async () => {
    ...
    try {
      ...
      const validateResponse = await fetch(`${api.getApiBaseUrl()}/validate-twilio-credentials`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      // Save the account
      const response = await fetch(`${api.getApiBaseUrl()}/twilio/accounts`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          userId: user.id,
  ...
  const handleDeleteAccount = async (accountId: string) => {
    ...
    try {
      const response = await fetch(`${api.getApiBaseUrl()}/twilio/accounts/${accountId}?userId=${user.id}`, {
        method: 'DELETE',
      });
  ...
  const handleTestAccount = async (account: TwilioAccount) => {
    try {
      const response = await fetch(`${api.getApiBaseUrl()}/validate-twilio-credentials`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
                        ...
                        <div className="space-y-3">
                          <div className="flex flex-wrap gap-2">
                            ...
                            <button
                              ...
                              className={`px-4 py-2 rounded-lg font-semibold transition ${
                                addingNumber || !selectedNumberToAdd
                                  ? 'bg-gray-600 cursor-not-allowed'
                                  : 'bg-emerald-600 hover:bg-emerald-700'
                              }`}
                            >
