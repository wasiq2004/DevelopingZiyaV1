...
async function generateTTS(text, options = {}) {
    // TEMPORARY: Force ElevenLabs for all voices until Sarvam conversion is fixed
    console.log(`[TTS Controller] Forcing ElevenLabs for reliable audio`);

    // Map Sarvam voice names to ElevenLabs voices
    const voiceMapping = {
        'abhilash': '21m00Tcm4TlvDq8ikWAM', // Rachel
        'anushka': 'EXAVITQu4vr4xnSDxMaL', // Bella
        'vidya': 'ThT5KcBeYPX3keUQqHPh', // Dorothy
        'meera': 'jsCqWAovK2LkecY7zXl4', // Freya
        'chitra': 'XB0fDUnXU5powFXDhCwa', // Charlotte
    };

    const voiceId = voiceMapping[options.voiceId] || voiceMapping[options.speaker] || options.voiceId || '21m00Tcm4TlvDq8ikWAM';
